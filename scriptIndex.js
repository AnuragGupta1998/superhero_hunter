...
function getSearchedSuperHeroes() {
	var displaySearchedHeroesResult = document.getElementById(
		'displayHeroSearchResults'
	);
...
`;
	...
	let url =
		'https://superheroapi.com/api.php/1500237820345742/search/' +
		searchBar.value;
	...
	apiRequest.onreadystatechange = function () {
		...
		if (this.readyState == 4 && this.status == 200) {
			...
			if (data.response === 'error') {
				displaySearchedHeroesResult.innerHTML =
					'<h2 class = "loading-and-error-font"> Oops ! There is no super hero with such name, please make sure that you are spelling it correctly :/ </h2> ';
				return;
			}

			//exit if searched query does not match the displayed result
			...
			for (let hero of data.results) {
				...
				favButton.onclick = function (event) {
					toggleFavourite(event, hero.id, favButton);
				};
...
function toggleFavourite(event, heroID, favButton) {
	...
	//delete it , if it already exists, else add it to favorites
